...
app.get('/loginpage.html', (req, res) => {
  const username = req.session.username || "Se connecter";
  res.render('loginpage.ejs', {username: username, error_login: false, error_register: false });
});

app.post('/loginpage.html', async (req, res) => {
  if (!req.body.registerUsername) { // user wants to login
    let user = await User.findOne({ username: req.body.loginUsername, password: req.body.loginPassword });
    if (user) { // verify user in DataBase
      req.session.username = user.username;
      let quizzData = await Quizz.find().toArray();
      res.render('mainpage.ejs', { username: req.session.username, data: quizzData });
    } else {
      const username = req.session.username || "Se connecter";
      res.render('loginpage.ejs', { username: username, error_login: true, error_register: false });
    }
  }
  
  else { // user wants to register
    // add new user to the database
    let verify_username = await User.findOne({ username: req.body.registerUsername });
    if (!verify_username) { // verify if username is not already used
      ...
      res.render('mainpage.ejs', { username: req.session.username, data: quizzData });
        ...
        const username = req.session.username || "Se connecter";
        res.render('loginpage.ejs', { username: username, error_login: false, error_register: true });
    }
  }
});
...
app.get('/creationpage.html', (req, res) => {
  ...
  else { // if not connected, redirect to login page
    res.render('loginpage.ejs', {username: "Se connecter", error_login: false, error_register: false });
  }
});
